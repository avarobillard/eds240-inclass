...
mko_clean %>% 
  # set an order to categorical data- set to factor
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) %>% 
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_density(alpha = 0.5, adjust = 0.5) # adjust curve smoothness- higher is more smooth
```
...
mko_clean %>% 
  mutate(month_name = factor(month_name, levels = rev(month.name))) %>% # reverse order of factors
  ggplot(aes(x = Temp_bot, y = month_name)) +
  geom_density_ridges(rel_min_height = 0.01)  # removes ends of ridges
...
ggplot(mko_clean, aes(x = month_name, y = Temp_bot)) +
  ...
  coord_flip() # make horizontal to fix long name issue
```
...
ggplot(penguins, aes(x = species, y = body_mass_g, color = factor(year))) + # to treat year as groups
  geom_boxplot(outlier.shape = NA)+ 
