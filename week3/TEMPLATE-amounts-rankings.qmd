...
sui_severity <- ca_region |>
  ...
  mutate(sui_category = factor(sui_category, 
                               levels = c("Very low", "Low",
                                          "Moderate", "High", "Severe")))# order months Oct > Sep (water year)
```
...
ggplot(sui_severity, aes(x = month, y = tot_num_sui, fill = sui_category)) +
  geom_col(position = "fill") + # makes values range 0-1
  scale_fill_manual(values = sui_colors) +
  scale_y_continuous(labels = scales::label_percent(scale = 100))  # make into percents
```
...
order_2015 <- heatmap_data |> 
  filter(year == 2015) |> 
  arrange(annual_mean_sui) |> # arrange by values in this column
...
heatmap_order <- heatmap_data |> 
  ...
  mutate(subregion_name = fct_reorder(.f = subregion_name, .x = order)) # reorder subregion by order in order column
```
...
ggplot(heatmap_order, aes(x = year, y = subregion_name, fill = annual_mean_sui)) +
  ...
  guides(fill = guide_colorbar(title.position = "top", barwidth = 15, barheight = 0.75)) +
